...
7. On a **tablet or phone**: **drag** on the target to aim, **tap** it to shoot. On devices with motion sensors a **Tilt aiming** checkbox appears in the Aiming section — enable it (iOS asks for motion-sensor permission) and the device becomes the bow: **dip it down toward horizontal to grab the bow**, then **raise it toward vertical** to bring the aim up onto the target, and **roll** it left/right for windage. After every shot the bow is released — dip the device down again to grab it for the next arrow, like re-raising a real bow. While tilt-aiming, a thumb-drag on the target fine-tunes the aim (handy for left/right corrections). Motion sensors require HTTPS, so tilt works on the hosted site but not from a local `file://` open on iOS.
...
| Ready banner (top-left, tilt mode only) | Orange = bow released, dip the device toward horizontal to grab it; green = bow in hand, raise to aim and shoot. The reticle is also grey while released, green when ready. |
...
Tilt is **gravity-referenced and absolute**, with a grab/release cycle that mirrors the real shot cycle:

- **Release** — after every shot (and on enable / distance change) the bow is released: tilt input detaches and the usual lowering glide brings the cursor to the bottom edge.
- **Grab** — dipping the device to ≤ `TILT.grabPitch` = 30° above horizontal grabs the bow: the aim attaches at the bottom edge, and the device's roll at that moment becomes the windage anchor.
- **Aim** — while grabbed, pitch maps absolutely to elevation: raising the device through `TILT.elevRange` = 30° sweeps the aim bottom → top (target center ≈ 45° device pitch). Roll relative to the grab anchor maps to windage over `TILT.windRange` = 32° — deliberately gentle, since rolling a tablet precisely is the hardest motion.

Because the pitch reference is gravity itself and the roll anchor resets at every grab, there is no calibration to drift — each arrow starts from a clean anchor. Sensor readings are low-pass filtered in the aim loop (`TILT.smooth` = 8 /s) to suppress hand jitter. Simulated *Bow droop & sway* is **not** applied in tilt mode — the hand's own tremor provides the hold instability. While the bow is grabbed, tilt holds the aim (arrow keys are overridden), but **dragging on the target trims the tilt anchors** — a thumb-drag nudges the windage anchor and elevation offset for fine corrections that are hard to do by rolling. Tap the target or press SHOOT to loose. Pitch and roll are computed from the **gravity vector** reconstructed from `beta`/`gamma` and projected onto the screen axes per `screen.orientation.angle` — raw Euler angles flip representation when the device passes vertical (pitch 95° reports as `beta=180, gamma=−85`), which would fake a dip-to-grab; the gravity vector is continuous there. Portrait and both landscape orientations all work.
...
| `TILT.grabPitch` / `elevRange` / `windRange` / `smooth` | 30° / 30° / 32° / 8 /s | Tilt grab threshold, pitch→elevation and roll→windage spans, sensor low-pass |
