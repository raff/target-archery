...
7. On a **tablet or phone**: **drag** on the target to aim, **tap** it to shoot. On devices with motion sensors a **Tilt aiming** checkbox appears in the Aiming section — enable it (iOS asks for motion-sensor permission) and the device becomes the bow, relative to however you hold it: the pose where you grab the bow is the bottom of the aim range, **raising the device ~30° from there** sweeps the aim up onto the target, and **rolling** it left/right moves the windage. After every shot the bow is released — **dip the device back down near your grab pose** to take it in hand for the next arrow, like re-raising a real bow (the first grab after enabling is immediate). While tilt-aiming, a thumb-drag on the target fine-tunes the aim (handy for left/right corrections). Motion sensors require HTTPS, so tilt works on the hosted site but not from a local `file://` open on iOS.
...
- **Grab** — the pose where the bow is grabbed becomes the bottom of the aim range: pitch and roll at that moment are the anchors. The first grab after enabling is immediate; later grabs require dipping back to within `TILT.regrabSlack` = 5° of the previous grab pose. Dipping below the anchor while grabbed lowers the range floor with it.
- **Aim** — while grabbed, raising the device through `TILT.elevRange` = 30° above the anchor sweeps the aim bottom → top. Roll relative to the grab anchor maps to windage over `TILT.windRange` = 32° — deliberately gentle, since rolling a tablet precisely is the hardest motion.
- **Release** — after every shot (and on distance change) the bow is released: tilt input detaches and the usual lowering glide brings the cursor to the bottom edge. Dip back down near the grab pose to re-grab.

Because both anchors reset at every grab, any holding angle works — upright like a bow or low on the lap — and there is no calibration to drift. Sensor readings are low-pass filtered in the aim loop (`TILT.smooth` = 8 /s) to suppress hand jitter. Simulated *Bow droop & sway* is **not** applied in tilt mode — the hand's own tremor provides the hold instability. While the bow is grabbed, tilt holds the aim (arrow keys are overridden), but **dragging on the target trims the tilt anchors** — a thumb-drag nudges the windage anchor and elevation offset for fine corrections that are hard to do by rolling. Tap the target or press SHOOT to loose. Pitch and roll are computed from the **gravity vector** reconstructed from `beta`/`gamma` and projected onto the screen axes for the current rotation — raw Euler angles flip representation when the device passes vertical (pitch 95° reports as `beta=180, gamma=−85`), which would fake a dip-to-grab; the gravity vector is continuous there. The screen rotation comes from the legacy `window.orientation` on iOS rather than `screen.orientation.angle`: iPadOS can reference the latter to landscape while the tilt sensor stays portrait-referenced, which swapped the aim axes in portrait. Portrait and both landscape orientations all work.
