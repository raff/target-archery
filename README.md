...
7. On a **tablet or phone**: **drag** anywhere on the range to aim, **tap** it to shoot. On small screens (phones) the control panel is tucked behind the **⚙ Settings** button in the bottom bar, which also keeps the **distance selector** and a **Shoot** button always at hand. On devices with motion sensors, **tilt aiming starts enabled** — the sight dialog's **Start** button doubles as the user gesture for the iOS motion-permission prompt — and can be toggled with the **Tilt aiming** checkbox in the Aiming section. The pose you hold the device in when tilt engages becomes the center of the aim range for the session: **raise/lower the device ±15°** to sweep the aim, **roll** it left/right for windage. After every shot the bow is released — **dip the device ~10–15° down** to take it in hand for the next arrow, then raise it back onto the target, like re-raising a real bow; you can only shoot while **READY**. Raising back to the same pose returns the aim to the same spot. Toggle the checkbox off/on to re-center on a new pose; rotating the device re-centers automatically and reopens the sight dialog. While tilt-aiming, a thumb-drag on the target fine-tunes the aim (handy for left/right corrections). Motion sensors require HTTPS, so tilt works on the hosted site but not from a local `file://` open on iOS.
...
| Ready banner (top-left, tilt mode only) | Orange = bow released, dip the device down near your grab pose to take it; green = bow in hand, raise to aim and shoot. The reticle is also grey while released, green when ready. |
...
Pointer events on `#target-area` (the whole range, not just the canvas, so taps near the screen edges count) provide a keyboard-free aiming path: dragging moves the cursor **1:1** with the finger/mouse (the per-pixel angle is `atan(1/FOCAL)`, scaled by the canvas CSS size so it stays 1:1 when the canvas shrinks on small screens), and a release with ≤ 6 px of total movement counts as a **tap → shoot**. Dragging past the tap threshold cancels the post-shot lowering glide, same as the arrow keys. `touch-action: none` on the range area keeps drags from scrolling the page.
...
- **Anchor** — the first grab (immediate on enabling) centers the aim range on that pose: the range floor is set `TILT.elevRange`/2 = 15° below it. The pitch anchor then stays **fixed for the session**, so raising the device back to the same pose always returns the aim to the same spot; toggling tilt off/on re-centers on a new pose. (The floor must not follow the re-grab dip down — that made the range bottom out at the lowest dip point, far below the actual aiming pose.)
- **Aim** — while grabbed, raising the device through `TILT.elevRange` = 30° above the range floor sweeps the aim bottom → top. Roll relative to its per-grab anchor maps to windage over `TILT.windRange` = 32° — deliberately gentle, since rolling a tablet precisely is the hardest motion.
- **Release / re-grab** — after every shot (and on distance change) the bow is released: tilt input detaches and the usual lowering glide brings the cursor to the bottom edge. Dipping to within `TILT.regrabSlack` = 5° of the range floor (~10–15° below the aiming pose) grabs the bow again. **Shooting is blocked while the bow is released** — Space, taps and SHOOT all require READY in tilt mode.

Any holding angle works — upright like a bow or low on the lap — and there is no calibration to drift: pitch is referenced to the session anchor and roll re-anchors at every grab. Sensor readings are low-pass filtered in the aim loop (`TILT.smooth` = 8 /s) to suppress hand jitter. Simulated *Bow droop & sway* is **not** applied in tilt mode — the hand's own tremor provides the hold instability. While the bow is grabbed, tilt holds the aim (arrow keys are overridden), but **dragging on the target trims the tilt anchors** — a thumb-drag nudges the windage anchor and elevation offset for fine corrections that are hard to do by rolling. Tap the target or press SHOOT to loose. Pitch and roll are computed from the **gravity vector** reconstructed from `beta`/`gamma` and projected onto the screen axes — raw Euler angles flip representation when the device passes vertical (pitch 95° reports as `beta=180, gamma=−85`), which would fake a dip-to-grab; the gravity vector is continuous there.

The screen orientation is **auto-detected from gravity too**, not from a browser API: in any playable hold, the device axis best aligned with "up" is screen-up. Detection and axis mapping share one convention by construction, so the aim directions cannot come out reversed — `screen.orientation.angle` and `window.orientation` are reported inconsistently on iPadOS (the angle can be referenced to landscape while the sensor frame stays portrait) and cost several rounds of swapped/inverted axes. The latched orientation only updates on a clear reading (device raised > ~20°, one axis clearly dominant), so aiming banks can't flip it; when it does flip — the device was physically rotated — the tilt session re-centers on the next reading and the sight dialog reopens for a clean re-entry (a stale anchor from the old orientation used to make the bow ungrabbable after rotating).

A **ready banner** (top-left of the range) reflects the grab state while tilt is enabled: orange *"dip device flat to grab the bow"* when released, green *"READY — raise to aim"* once grabbed. The aiming reticle also dims grey while the bow is released and turns green when in hand.
...
The target canvases scale down via `width: min(500px, 94vw, 80vh)` (drawing stays at 500 × 500 internal pixels; pointer math compensates by the CSS scale); the `vh` term keeps the face on-screen in short windows such as landscape phones. Below 700 px viewport width the layout switches to a phone arrangement instead of stacking:

- The **target area fills the screen** — no scrolling. The body height uses `100dvh` so iOS Safari's collapsing toolbar can't hide the bottom of the page, with `env(safe-area-inset-bottom)` padding on the bottom edges.
- The **controls panel becomes a slide-up Settings sheet** (`position: fixed`, max 78 vh, scrollable, closed by its ✕ button or by tapping the dimmed backdrop). Open it with the **⚙ Settings** button.
- A **bottom bar** keeps the essentials in reach: a distance selector (kept in sync with the one inside Settings — changing either runs the same distance-change flow), a **Shoot** button, and the Settings button.
- The canvas caps at `48vh` and top-aligns, so when the sight/start dialog is open the full target face — and the cyan zero marker — stays visible above it. The status banners (READY / wind) drop below the distance label, and the flight side view shrinks to 110 px (the canvas re-reads its CSS height in `drawFlight`).
...
| `TILT.regrabSlack` / `elevRange` / `windRange` / `smooth` | 5° / 30° / 32° / 8 /s | Tilt re-grab dip tolerance, pitch→elevation and roll→windage spans, sensor low-pass |
