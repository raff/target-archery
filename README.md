...
7. On a **tablet or phone**: **drag** on the target to aim, **tap** it to shoot. On devices with motion sensors a **Tilt aiming** checkbox appears in the Aiming section — enable it (iOS asks for motion-sensor permission) **while holding the device the way you'll aim**, and that pose becomes the center of the aim range for the session: **raise/lower the device ±15°** to sweep the aim, **roll** it left/right for windage. After every shot the bow is released — **dip the device ~10–15° down** to take it in hand for the next arrow, then raise it back onto the target, like re-raising a real bow. Raising back to the same pose returns the aim to the same spot. Toggle the checkbox off/on to re-center on a new pose. While tilt-aiming, a thumb-drag on the target fine-tunes the aim (handy for left/right corrections). Motion sensors require HTTPS, so tilt works on the hosted site but not from a local `file://` open on iOS.
...
On touch devices that expose `DeviceOrientationEvent`, a **Tilt aiming** checkbox appears (on iOS 13+ enabling it calls `DeviceOrientationEvent.requestPermission()`, which must come from a user gesture — the checkbox tap).
...
- **Release / re-grab** — after every shot (and on distance change) the bow is released: tilt input detaches and the usual lowering glide brings the cursor to the bottom edge. Dipping to within `TILT.regrabSlack` = 5° of the range floor (~10–15° below the aiming pose) grabs the bow again.

Any holding angle works — upright like a bow or low on the lap — and there is no calibration to drift: pitch is referenced to the session anchor and roll re-anchors at every grab. Sensor readings are low-pass filtered in the aim loop (`TILT.smooth` = 8 /s) to suppress hand jitter. Simulated *Bow droop & sway* is **not** applied in tilt mode — the hand's own tremor provides the hold instability. While the bow is grabbed, tilt holds the aim (arrow keys are overridden), but **dragging on the target trims the tilt anchors** — a thumb-drag nudges the windage anchor and elevation offset for fine corrections that are hard to do by rolling. Tap the target or press SHOOT to loose. Pitch and roll are computed from the **gravity vector** reconstructed from `beta`/`gamma` and projected onto the screen axes for the current rotation — raw Euler angles flip representation when the device passes vertical (pitch 95° reports as `beta=180, gamma=−85`), which would fake a dip-to-grab; the gravity vector is continuous there. The screen rotation comes from the legacy `window.orientation` on iOS rather than `screen.orientation.angle`: iPadOS can reference the latter to landscape while the tilt sensor stays portrait-referenced, which swapped the aim axes in portrait. Portrait and both landscape orientations all work.
